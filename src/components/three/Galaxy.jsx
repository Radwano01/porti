...
const geometry = useMemo(() => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Purple center → Blue outer
    const t = Math.pow(r / radius, 1.7);
    const color = purple.clone().lerp(blue, t * 0.6);

    // -------------------------
    // Brightness equation: every 10th star is brighter
    // -------------------------
    const brightnessMultiplier = (i % 10 === 0) ? 1.5 : 1;
    colors[i3] = baseColors.current[i3] = THREE.MathUtils.clamp(color.r * brightnessMultiplier, 0, 1);
    colors[i3 + 1] = baseColors.current[i3 + 1] = THREE.MathUtils.clamp(color.g * brightnessMultiplier, 0, 1);
    colors[i3 + 2] = baseColors.current[i3 + 2] = THREE.MathUtils.clamp(color.b * brightnessMultiplier, 0, 1);
  }
